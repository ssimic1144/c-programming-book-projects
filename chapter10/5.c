...
void analyze_hand(){
    ...
    /* since cards are sorted we need to check if first card is T(or value 8) */
    ...
    /*since hand is sorted we need last and first card for "ace-low" straight*/
    int last_card = hand[4][0];
    int first_card = hand[0][0];
    /* this will only matter if we are checking for "ace-low" */ 
    if(last_card == 12 && first_card == 0){
        num_consec++;
    }
